# Развёртывание Mini App и Telegram Reader на BotHost
...
Для входа и управления источниками добавить только пять обязательных
...
Шестая переменная является отдельным аварийным выключателем Reader. Пока
источники не проверены, её не добавлять (без неё безопасное значение —
`false`). После ручной проверки первого источника добавить:

```dotenv
COLLECTOR_ENABLED=true
```

После изменения переменной нужен новый деплой. Удаление переменной или
значение `false` отключает постоянное live-чтение. Ручная кнопка «Проверить»
в Mini App всё равно может кратко подключиться к Telegram только для сверки
конкретного источника; сообщения при этом не читаются.
...
файл, а интерфейс покажет «Сессия сохранена» и каталог источников.

Для каждого источника порядок одинаковый:

1. Нажать «Открыть в TG» и вручную вступить в группу или подписаться на канал
   официальным приложением Telegram.
2. Вернуться в Mini App и нажать «Проверить». Сервер сверит публичный username,
   тип источника, числовой ID, членство и ожидаемый аккаунт. Источник останется
   выключенным.
3. Отдельно нажать «Включить».
4. Когда первый источник готов, задать `COLLECTOR_ENABLED=true` и выполнить
   новый деплой BotHost.

Reader принимает только новые сообщения и их правки из числового allowlist.
Он не вступает автоматически, не читает историю, не запрашивает участников и
не отправляет сообщения. Сначала событие сохраняется в SQLite inbox, затем
передаётся фильтру намерения; после перезапуска необработанная очередь не
теряется.
